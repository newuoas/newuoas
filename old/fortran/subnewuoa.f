      SUBROUTINE SUBNEWUOA(N,NPT,X,RHOBEG,RHOEND,IPRINT,MAXFUN,W,WHN,
     1 WHX,WHF,WHNF,SUCCESS,BS)
      IMPLICIT NONE
      INTEGER(KIND=4), INTENT(IN) :: N, NPT, IPRINT, MAXFUN, WHN
      INTEGER(KIND=4), INTENT(INOUT) :: WHNF
      INTEGER(KIND=4), INTENT(OUT) :: SUCCESS
      REAL(KIND=8), INTENT(IN) :: RHOBEG, RHOEND, WHX, BS(WHN,N)
      REAL(KIND=8), INTENT(INOUT) :: X, WHF, W(*) 

      INTEGER(KIND=4) :: NDIM, IXB, IXO, IXN, IXP, IFV, IGQ, IHQ, IPQ,
     1 IBMAT, IZMAT, ID, IVL, IW, NP, NPTM
C
C
...
C
      CALL SUBNEWUOB (N,NPT,X,RHOBEG,RHOEND,IPRINT,MAXFUN,W(IXB),
     1  W(IXO),W(IXN),W(IXP),W(IFV),W(IGQ),W(IHQ),W(IPQ),W(IBMAT),
     2  W(IZMAT),NDIM,W(ID),W(IVL),W(IW),WHN,WHX,WHF,WHNF,SUCCESS,BS)
   20 RETURN
      END 
