      SUBROUTINE SUBNEWUOB (N,NPT,X,RHOBEG,RHOEND,IPRINT,MAXFUN,XBASE,
     1  XOPT,XNEW,XPT,FVAL,GQ,HQ,PQ,BMAT,ZMAT,NDIM,D,VLAG,W,WHN,WHX,
     1  WHF,WHNF,SUCCESS,BS)
      USE STATISTICS
      IMPLICIT NONE
CCCCC DUMMIES CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      INTEGER(KIND=4), INTENT(IN) :: N, NPT, NDIM, IPRINT,
     1 MAXFUN, WHN
      INTEGER(KIND=4), INTENT(INOUT) :: WHNF
      INTEGER(KIND=4), INTENT(OUT) :: SUCCESS
      REAL(KIND=8), INTENT(IN) :: RHOBEG, RHOEND, WHX(WHN),BS(WHN,N)

      REAL(KIND=8), INTENT(INOUT) :: X(N), XBASE(N), XOPT(N), 
     1 XNEW(N), XPT(NPT,N), FVAL(NPT), GQ(N), HQ((N*(N+1))/2), 
     1 PQ(NPT), BMAT(NDIM,N), ZMAT(NPT,NPT-N-1),D(N), VLAG(*),W(*),WHF
C
...
C       10*NDIM = 10*(NPT+N).
C
CCCCC VARIABLES USED BY NEWUOB CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
C
  100 KNEW=0
     ...
     1  W(NP+N),W(NP+2*N),CRVMIN)
      ...
      CALL HSSD(N, NPT, HD, HQ, PQ, XPT, D)
      VQUAD = DOT_PRODUCT(D,GQ) + DOT_PRODUCT(HD,XOPT+HALF*D)
...
C      IF (DNORM .LT. HALF*RHO .OR. VQUAD >= -1.0D-16) THEN
          WRITE(*,*) "Short TR."
          WRITE(*,*) "DNORM"
          WRITE(*,*) DNORM
          WRITE(*,*) "DELTA"
          ...
          IF (TEMP .LE. DMAX1(DIFFA,DIFFB,DIFFC)) THEN
              GOTO 460
          ELSE
              WRITE(*,*) "3 Short TRs."
              GOTO 490
...
C
  ...
  310 XTEMP = WHX
      DO I = 1, N
         XTEMP = XTEMP + X(I)*BS(:,I)
      END DO
      IF (NF == 1) THEN
          F = WHF
      ELSE
          IF (NF .GT. NFTEST) THEN
             NF=NF-1
  ...
  320        FORMAT (/4X,'Return from NEWUOA because CALFUN has been',
     1      ' called MAXFUN times.')
             GOTO 530
          END IF
          CALL CALFUN (WHN,XTEMP,F)
      END IF
      IF (IPRINT .EQ. 3) THEN
          PRINT 330, NF-1,F,(X(I),I=1,N)
  330      FORMAT (/4X,'Function number',I6,
     1       '    F =',1PD18.10,
     1       '    The corresponding X is:'/(2X,5D15.6))
          PRINT 331, WHNF+NF-1,F,(XTEMP(I),I=1,WHN)
  331      FORMAT (/4X,'Function number',I6,
     1       '    F =',1PD18.10,
     1       '    The corresponding X is:'/(2X,5D15.6))
      END IF
...
C Theoretically, VQUAD should always be negative if this "if" is
C reached. But it is not the case in practice, because of computer
C rounding errors. For example, if the interpolation points have just
C been shifted (i.e., XBASE has just been changed) before this "if",
C and if the theoretical value of VQUAD is very small (close to machine
C precision), then possibly the computational value of VQUAD will be
C positive. This happened in numerical experiments.
  ...
  370     FORMAT (/4X,'Return from NEWUOA because a trust',
     1      ' region step has failed to reduce Q.')
      ...
      IF (RATIO .LE. TENTH) THEN
          DELTA=HALF*DNORM
      WRITE(*,*) "RATIO<0.1, HALF"
      ELSE IF (RATIO. LE. 0.7D0) THEN
          DELTA=DMAX1(HALF*DELTA,DNORM)
      WRITE(*,*) "0.1<RATIO<0.7, ONE"
      ELSE
...
C
  ...
  520         FORMAT (4X,'Least value of F =',1PD23.15,9X,
     1          'The corresponding X is:'/(2X,5D15.6))
              PRINT 521, WHNF+NF-1,F,(XTEMP(I),I=1,WHN)
  521         FORMAT (/4X,'Function number',I6,
     1        '    F =',1PD18.10,
     1        '    The corresponding X is:'/(2X,5D15.6))
          END IF
          GOTO 90
...
C
  ...
  540     X(I)=XBASE(I)+XOPT(I)
      ...
      IF (IPRINT .GE. 1) THEN
          PRINT 550, NF
  550     FORMAT (/4X,'At the RETURN from SUBNEWUOA',5X,
     1      'Number of function values =',I6)
          PRINT 520, F,(X(I),I=1,N)
          PRINT 551, WHNF+NF-1,F,(XTEMP(I),I=1,WHN)
  551     FORMAT (/4X,'Function number',I6,
     1    '    F =',1PD18.10,
     1    '    The corresponding X is:'/(2X,5D15.6))
      END IF
      IF (F < WHF) THEN
