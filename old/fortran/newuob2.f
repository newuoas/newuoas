      SUBROUTINE NEWUOB2 (N,NPT,X,RHOBEG,RHOEND,IPRINT,MAXFUN,XBASE,
...
C
  100 KNEW=0
CC      IF (ISUBUNSUC >= 1) THEN 
CC          IF (NEWITER == 1) THEN
CC             !!!!!!!!!!!!!!!! ?????????????????????!!!!!!!!!!!!!!!!!
CC             DELTA = DMAX1(RHO,1.0D-1*SUBRHOBEG) !!!! What should be the
CC                                                 !!!! first DELTA of the
CC                                                 !!!! "new" iterations?
CC             NEWITER = 0
CC          END IF
CC          GOTO 101
...
C      SUBNPT = 2*SUBN + 1
      ...
      SUBRHOEND = DMAX1(0.001D0*RHOEND,DMIN1(RHOEND,0.5D0**ITERNUM))
      !SUBRHOEND = DMAX1(1.0D-2*SUBRHOBEG, 1.0D-1*RHO)
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      SUBRHOBEG = MAX(RHO,DNORM,5.0D-1*SUBRHOBEG,SUBRHOEND)  !!!!! IMPORTANT
     ...
     1 SUBMAXFUN,SUBW,N,X,F,NFACT,SUBSUCCESS,SUBBS(:,1:SUBN))
      ...
      CALCULATED = 1
      FCALCULATED = F
      IF (DNORM <= 0.1D0*RHOEND) THEN   !!!!???????
          ISUBUNSUC = ISUBUNSUC + 1
          SUBSUCCESS = 0 !!!!!!!!!!!!??????????
          !GOTO 541
      END IF
      IF (ISUBUNSUC >= 3) GOTO 541
...
C      IF (DMAX1(DNORM, GNORM, RHO) <= RHOEND) THEN 
      ...
      ELSE 
          ...
          IF (SUBSUCCESS == 0) THEN
              !RHO = 0.1D0*RHO !!!!!!!!!??????????????
C              RHO = DMAX1(0.1D0*RHO,0.01D0*RHOEND) !!!!!!!!!!????
          ...
          ELSE
              !RHO = DMAX1(0.5D0*RHO,1.0D-1*RHOEND) !!!!!!!!!!????
               RHO = DMAX1(0.5D0*RHO,0.001D0*RHOEND) !!!!!!!!!!????
              !!!! What if when 0.1D0*RHO <= RHOEND, then break to "New"
              !!!! iterations?
          END IF
          GOTO 001
