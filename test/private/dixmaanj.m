...
function  varargout = dixmaanj( action, varargin )
...
case 'setup' % varargout = [ x0, fstar, xtype, xlower, xupper, clower, cupper, class, error ]
   ...
   if ( length( varargin ) )
      ...
      if ( n < 3 || round( n/3 ) ~= n/3 )
         disp( [ ' ERROR in dixmaanj: n = ', int2str(n),' does not satisfy n = 3*m!' ] )
      end
   else
      n = 12;
   end
...
case 'objf'   % varargout = [ f, g, H ]
   ...
   alpha = 1;  beta  = 0.625; gamma = 0.625; delta = 0.625; bigk  = [ 2, 0, 0, 2 ]; % case J
   varargout = opm_eval_cpsf( 'dixmaanj', 'elobjf', x, eldom, nargout, n, alpha, beta, gamma, delta, bigk );
