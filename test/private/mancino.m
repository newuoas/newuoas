...
case 'setup' % varargout = [ x0, fstar, xtype, xlower, xupper, clower, cupper, class, error ]
   ...
   if ( length( varargin ) )
      n = varargin{1};
      if ( n < 3 )
         disp( [ ' ERROR in mancino: n = ', int2str(n), ' must be > 2!' ] )
      end
   else
      n = 10;
   end
   varargout{1} = (1/n)*ones( n , 1 );          % x0
   varargout{2} = 0;                            % fstar (n-10)
   varargout{3} = '';                           % xtype
   varargout{4} = [];                           % xlower
   varargout{5} = [];                           % xupper
   varargout{6} = [];                           % clower
...
case 'objf'   % varargout = [ f, g, H ]
   ...
   x = varargin{1};
   n = length( x );
   varargout = opm_eval_cpsf( 'mancino', 'elobjf', x, { [ 1:n ] }, nargout, n );
