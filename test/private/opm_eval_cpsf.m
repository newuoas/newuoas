function fgHerr = opm_eval_cpsf( pname, fname, x, eldom, nout, varargin )
...
%   OUTPUT:
%
%   fgHerr: a cell array of length nout, with
%           fgHerr{1}: the function value at x
%           fgHerr{2}: the gradient value at x (if nout > 1)
%           fgHerr{3}: the Hessian  value at x (if nout > 2), as a sparse matrix.
%
...
for iel = 1:length( eldom )
   ...
   otherwise
      disp( ' OPM error: wrong number of arguments for opm_eval_cpsf!')
   end
end
