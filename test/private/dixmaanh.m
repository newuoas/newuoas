...
function  varargout = dixmaanh( action, varargin )
...
case 'setup' % varargout = [ x0, fstar, xtype, xlower, xupper, clower, cupper, class, error ]
   ...
   if ( length( varargin ) )
      ...
      if ( n < 3 || round( n/3 ) ~= n/3 )
         disp( [ ' ERROR in dixmaanh: n = ', int2str(n),' does not satisfy n = 3*m!' ] )
      end
   else
      n = 12;
   end
...
case 'objf'   % varargout = [ f, g, H ]
   ...
   alpha = 1;  beta  = 0.26 ; gamma = 0.26 ; delta = 0.26 ; bigk  = [ 1, 0, 0, 1 ]; % case H
   varargout = opm_eval_cpsf( 'dixmaanh', 'elobjf', x, eldom, nargout, n, alpha, beta, gamma, delta, bigk );
