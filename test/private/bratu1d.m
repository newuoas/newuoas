...
case 'setup' % varargout = [ x0, fstar, xtype, xlower, xupper, clower, cupper, class, error ]
   ...
   tenthh = 0.1/n+1;
   varargout{1} = [ 0;-[1:n-2]'.^2*tenthh; 0 ]; % x0
...
case 'objf'   % varargout = [ f, g, H ]
   ...
   varargout = opm_eval_cpsf( 'bratu1d', 'elobjf', x, eldom, nargout, n );
...
case 'elobjf' % varargout = [ fiel, giel, Hiel ]
   ...
   case 0
      ...
      if ( isnan( varargout{1} ) )
        disp( 'bratu1d: f is NaN' )
         keyboard
      end
      if ( nargout > 1 )
         dx2    = dx^2;
	 ...
	 if ( isnan(sum(varargout{2})) )
            disp( 'bratu1d: g is NaN' )
	    keyboard
	 end
         if ( nargout > 2 )
	    dx3     = dx^3;
