...
function  varargout = nmsqrtbls( action, varargin )

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%   The non-msqrbls problem.
%
...
case 'setup' % varargout = [ x0, fstar, xtype, xlower, xupper, clower, cupper, class, error ]
   ...
   if ( length( varargin ) )
      ...
      if ( n < 1 || round( sqrtn ) ~= sqrtn )
         disp( [ ' ERROR in msqrtals: n = ', int2str(n),' but should satisfy n >= 4, n = m^2!' ] )
      end
   else
      n = 16;
   end
   varargout{1} = 0.2*sin( ([1:n].^2)' );       % x0
   varargout{2} = 0.005755250;                  % fstar
   varargout{3} = '';                           % xtype
   varargout{4} = [];                           % xlower
   varargout{5} = [];                           % xupper
   varargout{6} = [];                           % clower
...
case 'objf'   % varargout = [ f, g, H ]
   ...
   A = B*B;
   varargout = opm_eval_cpsf( 'nmsqrtbls', 'elobjf', x, eldom, nargout, A(:) );
...
case 'elobjf' % varargout = [ fiel, giel, Hiel ]
   ...
   %  ievr is the position of the numerical values of vector indeces irow in the argument x
   %  ievc is the position of the numerical values of vector indeces icol in the argument x
   ...
   riel   = varargin{3}(iel) - x(pir)*ones(1,d)*x(ievc);
   varargout{1} = riel^2;
   if ( nargout > 1 )
      Jiel =  zeros( lx, 1 );
      for k = 1:d
         Jiel(pir    )= Jiel(pir)-x(ievc(k));
	 Jiel(ievc(k))= Jiel(ievc(k))-x(pir);
      end
      varargout{2} = 2*Jiel*riel;
      if ( nargout > 2 )
         Hiel = sparse( lx, lx );
         for k = 1:d
	    Hiel(ievc(k),pir) = Hiel(ievc(k),pir)-1;
	    Hiel(pir,ievc(k)) = Hiel(pir,ievc(k))-1;
	 end
