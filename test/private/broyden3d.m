...
case 'setup' % varargout = [ x0, fstar, xtype, xlower, xupper, clower, cupper, class, error ]
   ...
   varargout{2} = 0.0;                    % fstar (n=10) ????
   varargout{3} = '';                           % xtype
   varargout{4} = [  -Inf*ones( n, 1 ) ];% xlower
   varargout{5} = [   Inf*ones( n, 1 ) ];% xupper
   varargout{6} = [];                           % clower
   varargout{7} = [];                           % cupper
...
case 'objf'   % varargout = [ f, g, H ]
   ...
   varargout = opm_eval_cpsf( 'broyden3d', 'elobjf', [ 0; x; 0 ], eldom, nargout );

   %  Zero the derivatives of fixed variables.
   
   if ( nargout > 1 )
      varargout{2} = varargout{2}(2:end-1);
      if ( nargout > 2 )
         varargout{3} = varargout{3}(2:end-1, 2:end-1 );
      end
   end
