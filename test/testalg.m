...
% Remark on rng: all the calls of rng is to make sure that
...
for ip = 1 : np 
    ...
    for is = 1 : ns 
        ...
        for ir = 1 : nr 
            ...
            if (isfield(options, 'reproduce')) solv_options.reproduce = options.reproduce; end
            if (isfield(options, 'signif')) solv_options.signif = options.signif; end
            if (isfield(options, 'prec')) solv_options.prec = options.prec; end
            if (isfield(options, 'noise')) solv_options.noise = options.noise; end

            if (strcmpi(solv, 'newuoas3'))
                solv = 'newuoas';
                solv_options.maxsubspacedim = 3;
            elseif (strcmpi(solv, 'newuoas4'))
                solv = 'newuoas';
                solv_options.maxsubspacedim = min(n,4);
            elseif (strcmpi(solv, 'newuoas5'))
                solv = 'newuoas';
                solv_options.maxsubspacedim = min(n,5);
            elseif (strcmpi(solv, 'newuoas10'))
                solv = 'newuoas';
                solv_options.maxsubspacedim = min(n,10);
            elseif (strcmpi(solv, 'newuoas20'))
                solv = 'newuoas';
