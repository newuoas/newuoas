      SUBROUTINE TRSUBP (D, N, G, B, DELTA)
C min 0.5*D'*B*D + D'*G
C s.t. \|D\|<=DELTA
C B contains the lower trianugular part of B, in this order: B(1,1),
C B(2,1), B(2,2), B(3,1), B(3,2), B(3,3), ...
      IMPLICIT NONE
      INTEGER(kind = 4), intent(in) :: N
      REAL(kind = 8), intent(out) :: D(N)
      REAL(kind = 8), intent(in) :: G(N)
      REAL(kind = 8), intent(in) :: B(N*(N+1)/2) 
      REAL(kind = 8), intent(in) :: DELTA

      REAL(kind = 8) :: XOPT(N), XPT(1,N), PQ(1), W(4*N), CRVMIN
      INTEGER(kind = 4) :: NPT, NP
      NPT = 1
      NP = N+1
      XOPT = 0.0D0
      XPT = 0.0D0
      PQ = 0.0D0
      CALL TRSAPP (N,NPT,XOPT,XPT,G,B,PQ,DELTA,D,W,W(NP),
     ...
     1  D,G,HD,HS,CRVMIN)
      ...
      DIMENSION XOPT(N),XPT(NPT,N),GQ(N),HQ(N*(N+1)/2),PQ(NPT),STEP(N),
     1  D(N),G(N),HD(N),HS(N)
C
